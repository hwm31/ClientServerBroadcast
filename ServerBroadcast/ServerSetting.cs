...
namespace ServerBroadcast
{
    public partial class ServerSetting : MetroFramework.Forms.MetroForm
    {
        ...
        private void SaveButton1_Click(object sender, EventArgs e)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        bool isSelected = Convert.ToBoolean(row.Cells["colSele"].Value);
                        if (isSelected)
                        {
                            string location = row.Cells["colLoc"].Value.ToString();

                            string query = "INSERT INTO your_table_name (Location) VALUES (@location)";
                            using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                            {
        ...
        public List<string> GetColLocValues()
        {
            List<string> values = new List<string>();

            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (row.Cells["colLoc"].Value != null)
                {
                    values.Add(row.Cells["colLoc"].Value.ToString());
                }
            }
